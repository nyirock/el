...
//IPAddress server_addr(172,31,3,90); //:( Didn't work -queried as hostname variable
...
void loop() {  
  ...
  //adjusted for timezone, manually for -6 hours
  //String query = "INSERT INTO sql3126451.temp_humid_2sensors_timestamp_TST VALUES (NULL, DATE_ADD(NOW(), INTERVAL -6 HOUR),"+data1+","+data2+")";
  //slightly more elegantly with time_zone conversion
  String query = "INSERT INTO sql3126451.temp_humid_2sensors_timestamp_TST VALUES (NULL, convert_tz(now(), \'+0:00\', \'-6:00\'),"+data1+","+data2+")";
  //named zones doesn't work on the server because probably not set up properly
  query.toCharArray(q, 198);
  my_conn.cmd_query(q);
